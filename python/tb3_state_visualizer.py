...
DEFAULT_JOINT_VEL_YMIN = -30.0
DEFAULT_JOINT_VEL_YMAX = 30.0
...
class Tb3StateWindow(QMainWindow):
    def __init__(self, pdu_manager: PduManager, robot_name: str):
        ...
        self.joint_vel_ymin = _get_env_float("TB3_STATE_JOINT_VEL_YMIN", DEFAULT_JOINT_VEL_YMIN)
        self.joint_vel_ymax = _get_env_float("TB3_STATE_JOINT_VEL_YMAX", DEFAULT_JOINT_VEL_YMAX)
        ...
        self.ax_world = self.figure.add_subplot(221)
        self.ax_joint = self.figure.add_subplot(222)
        self.ax_imu = self.figure.add_subplot(223)
        self.ax_tf = self.figure.add_subplot(224)
    ...
    def _draw_world(self):
        ...
        if self.last_odom is None:
            ax.text(0.5, 0.5, "waiting for odom", ha="center", va="center", transform=ax.transAxes)
            return
    ...
    def _draw_joint(self):
        ...
        if not self.enable_joints:
            ax.text(0.5, 0.5, "joint_states disabled", ha="center", va="center", transform=ax.transAxes)
            return
        if self.joint_decode_failed:
            ax.text(0.5, 0.5, "joint_states decode failed", ha="center", va="center", transform=ax.transAxes)
            return
        if self.last_joint_state is None or len(self.last_joint_state["names"]) == 0:
            ax.text(0.5, 0.5, "waiting for joint_states", ha="center", va="center", transform=ax.transAxes)
            return

        names = self.last_joint_state["names"]
        velocities = [
            0.0 if abs(value) < self.joint_vel_deadband else value
            for value in self.last_joint_state["velocity"]
        ]
        positions = self.last_joint_state["position"]
        xs = np.arange(len(names))
        ax.bar(xs, velocities, width=0.45, label="velocity")
        ax.set_xticks(xs)
        ax.set_xticklabels(names, rotation=10)
        ax.set_ylim(self.joint_vel_ymin, self.joint_vel_ymax)
        ax.set_ylabel("rad/s")
        ax.legend(loc="upper right")
        lines = [
            f"{name}: pos={position:.3f} rad  vel={velocity:.3f} rad/s"
    ...
    def _draw_imu(self):
        ...
        if self.last_imu is None:
            ax.text(0.5, 0.5, "waiting for imu", ha="center", va="center", transform=ax.transAxes)
            return
    ...
    def _draw_tf(self):
        ...
        if self.last_tf is None or len(self.last_tf.transforms) == 0:
            ax.text(0.5, 0.5, "waiting for tf", ha="center", va="center", transform=ax.transAxes)
            return
        ...
        for tr in self.last_tf.transforms:
            lines.append(
                ...
                f"({tr.transform.translation.x:.3f}, {tr.transform.translation.y:.3f}, {tr.transform.translation.z:.3f})"
            )
        ax.text(0.03, 0.95, "\n".join(lines), va="top", ha="left", family="monospace")
