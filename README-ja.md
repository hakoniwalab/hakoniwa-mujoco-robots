...
- `HAKO_FORKLIFT_RESUME_CMD_HOLD_SEC`（既定: `2.0`）
  - 原因: Python controller が tick非同期（real-time loop）で動くため、復帰境界で入力レースが起こり得る
  - 役割: そのレースを吸収する **temporary / transitional** な暫定バリア
  - 非本質: RD意味論の本体でも物理保証機構でもない
  - 根本解: Python controller を Hakoniwa asset化（tick同期）し、step/epoch整列で入力因果を揃える
  - 設計意図: 将来の縮小・撤廃（Roadmapに明記）
...
- 指標評価には `RESUME_CMD_HOLD_SEC` 区間も含める（運用手順を含む、より厳しい条件）
...
### Q10. `HAKO_FORKLIFT_RESUME_CMD_HOLD_SEC` はチートですか？
A. チートではなく、**temporary / transitional** な暫定バリアです。  
原因は Python controller の tick非同期実行による復帰境界の入力レースです。  
`RESUME_CMD_HOLD_SEC` はこのレース吸収が役割で、RD意味論の本体ではありません。  
根本解は Python controller の asset化（tick同期）で、Roadmap上は hold時間の縮小・撤廃を目標にしています。

### Q11. なぜ Phase2（荷物・棚・複雑接触込み）の復旧エビデンスを作らないのですか？
A. 現時点の目的が、RD前提となる **ExecutionUnit の継続性（Phase1）** の成立確認だからです。
本リポジトリは Data Plane 側の継続実装を提供します。RD制御面（commit-point連動保存、Ownership移譲規則、Epoch整合の受け渡し時点）と結合しない Phase2 は、必要工数に対する知見回収効率が低くなります。
...
- Python controller の asset化（tick同期）による入力レース解消と `RESUME_CMD_HOLD_SEC` の段階的撤廃
- 退避対象の段階的拡張（荷物・棚など）
