...
実装上の保存仕様:
- 状態ファイル形式は `v3`（後方互換で `v2` / `v1` 読み込み対応）
...
`forklift_unit` の再起動試験で、以下を確認済みです（2026-02-23 実施 / MuJoCo v3.5.0（`MUJOCO_VERSION.txt`）/ state format v3）。

- `logs/forklift-unit-recovery.log` に
  - `START restored=yes ... phase=2 ... target_v=-0.700000 ...`
- `logs/forklift-unit-run.log` に
  - `Resume control phase=2 target(v,yaw,lift)=(-0.7, ...)`
- 復帰後の `AUTOSAVE` でも `phase=2` 維持を確認

これにより、少なくとも現行スコープ（フォークリフト本体 + 制御状態）では  
**Phase2停止 -> Phase2復帰** が成立することを確認しています。
...
A. 段階的設計のためです。現在は「フォークリフト本体」と「制御状態」のみを保存対象としています。外部オブジェクト（棚・荷物）は将来の拡張対象です。

### Q5. MuJoCoの内部solver状態は保存していますか？
A. 保存していません。保存対象は `qpos` / `qvel` / `qacc` / 制御状態です。solver内部キャッシュやwarm-start状態は保存対象外です。これは物理連続性よりも実行継続性と再現性を優先した設計です。
