#!/usr/bin/env python3
"""Send a jump-like pose sequence to the Unitree Go1 Hakoniwa asset.

This is a posture demo, not verified jumping or locomotion.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Send a jump-like home/crouch/extend/recover pose sequence to Go1JointAsset."
    )
    ...
    parser.add_argument("--sender-asset-name", default="Go1PoseBounceSender")
    ...
    parser.add_argument("--endpoint-name", default="go1_pose_bounce_sender")
    parser.add_argument("--delta-usec", type=int, default=20_000)
    parser.add_argument("--send-rate-hz", type=float, default=40.0)
    parser.add_argument("--cycles", type=int, default=2)
    parser.add_argument("--hold-home-sec", type=float, default=0.4)
    parser.add_argument("--crouch-sec", type=float, default=0.45)
    parser.add_argument("--extend-sec", type=float, default=0.18)
    parser.add_argument("--recover-sec", type=float, default=0.55)
    parser.add_argument("--hold-recover-sec", type=float, default=0.35)
    parser.add_argument(
        "--no-read-joint-state",
        action="store_true",
...
def lerp_pose(start: list[float], end: list[float], ratio: float) -> list[float]:
...
def pose_at_time(elapsed_sec: float, args: argparse.Namespace) -> tuple[str, list[float], bool]:
    cycle_sec = (
        args.hold_home_sec
        + args.crouch_sec
        + args.extend_sec
        + args.recover_sec
        + args.hold_recover_sec
    )
    total_sec = cycle_sec * max(0, args.cycles)
    if elapsed_sec >= total_sec:
        return "done", list(HOME), True

    t = elapsed_sec % cycle_sec
    if t < args.hold_home_sec:
        return "home", list(HOME), False
    t -= args.hold_home_sec

    if t < args.crouch_sec:
        return "crouch", lerp_pose(HOME, CROUCH, t / args.crouch_sec), False
    t -= args.crouch_sec

    if t < args.extend_sec:
        return "extend", lerp_pose(CROUCH, EXTEND, t / args.extend_sec), False
    t -= args.extend_sec

    if t < args.recover_sec:
        return "recover", lerp_pose(EXTEND, HOME, t / args.recover_sec), False

    return "settle", list(HOME), False


def compact_joint_state(joint_state) -> str:
    wanted = {"FR_thigh", "FR_calf", "RL_thigh", "RL_calf"}
    parts: list[str] = []
...
def main() -> int:
    ...
    def on_manual_timing_control(_context):
        try:
            ...
            print(
                "[INFO] Go1 pose bounce sender callback started: "
                f"robot={args.target_robot_name} command={args.command_pdu_name} "
                f"cycles={args.cycles} order={','.join(JOINT_ORDER)}"
            )
            while not shutdown.is_set():
                ...
                if elapsed_usec >= next_send_usec:
                    ...
                    phase, targets, done = pose_at_time(elapsed_sec, args)
                    endpoint.send_by_name(command_key, make_payload(targets))
                    if (count % 10) == 0 or done:
                        message = (
                            f"sent pose phase={phase} "
                            f"FR_thigh={targets[1]:.3f} FR_calf={targets[2]:.3f} "
                            f"RL_thigh={targets[10]:.3f} RL_calf={targets[11]:.3f}"
                        )
            ...
            print(f"[ERROR] Go1 pose bounce sender callback failed: {exc}")
            callback_state["result"] = 1
            shutdown.set()
        return 0
    ...
    print("[INFO] Go1 pose bounce sender is registered.")
