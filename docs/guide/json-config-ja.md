...
| `pdu_config` | Hakoniwa PDU として出すための設定 | `pdu_name`, `update_rate_hz`, PDU message type |
...
現行の JSON は完全にはこの入れ子構造になっていません。
互換性のため、`spec` 相当のフィールドは top-level に置かれているものが多く、`mjcf_binding` 相当は `RuntimeBinding` というキー名で表現されています。
...
このガイドでは、説明上は `spec` / `mjcf_binding` / `pdu_config` で整理し、現行 JSON のキー名も併記します。
...
`pdu_config` は、MuJoCo state をどの PDU として、どの周期で出すかを表します。
...
`joint_name` は MJCF joint 名です。
`RuntimeBinding.actuator_name` は MJCF actuator 名です。
`type` は MJCF actuator 種別と合わせます。
...
- `mjcf_binding` / `RuntimeBinding.source_site` や `actuator_name` が意図した MJCF object か。
...
5. MJCF に合わせて `RuntimeBinding` や `mjcf_joint` を書く。
