...
5. MJCF 内の名前と JSON の `RuntimeBinding` が対応しているか確認する。
...
| 超音波センサの原点と向き | `<site name="front_ultrasonic_site">` | `RuntimeBinding.source_site` |
...
JSON の `type` と MJCF の actuator 種別は一致させます。
...
`joint_name` は joint の名前、`RuntimeBinding.actuator_name` は actuator の名前です。
...
- `RuntimeBinding.actuator_name` は `<position>`, `<velocity>`, `<motor>` の `name` と一致しているか。
- actuator JSON の `type` は MJCF actuator 種別と一致しているか。
...
- actuator JSON の `type` と MJCF actuator 種別が一致していない。
